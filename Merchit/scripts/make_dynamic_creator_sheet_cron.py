...
def makeCreatorSheetDynamic():
    ...
    print("INSIDE CREATOR SHEET DYNAMIC FUNCTION 3")

    with open('/home/info/M-AutoLog/Autolog 4.0/service_account.json') as source:
        info = json.load(source )
    credentials = service_account.Credentials.from_service_account_info(info)

    client = pygsheets.authorize(service_account_file='/home/info/M-AutoLog/Autolog 4.0/service_account.json')
    # client = pygsheets.authorize(service_account_file='service_account.json')

    print("GOT SERVICE ACCOUNT CREDS 4")
    ...
    for i in range(0 , len(sheet_names)):
        ...
        update_tracking_status(wksCreatorOrders , wks_client_baseprice , wksDSD , checktag_list[i])
